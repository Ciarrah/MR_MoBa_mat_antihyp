...
# install.packages("/Users/cb18669/Downloads/gwasglue-master", repos= NULL, type="source")


# Regression results script
...
outcomes_of_interest=gsub("_MBRN","",gsub("Beta_Off_","",outcomes_of_interest))
...
results_all_DC$`Upper CI`=-results_all_DC$`Upper CI`

results_all_DC$`Lower CI`=(results_all_DC$`Lower CI`/19.38)*10
results_all_DC$`Lower CI`=-results_all_DC$`Lower CI`
...
results_all_DC[which(results_all_DC$Outcom=="Hypertensive disorders of pregnancy"),c("Beta coefficient", "Upper CI", "Lower CI")]=
...
write.csv(results_all_DC ,"full_offspring_results.csv")

results_all_DC=read.csv("full_offspring_results.csv")
...
filename=paste0("hypertension_subclass_offspring_r&r.tiff")
...
results_table_Offspring_DC=results_table_Offspring_DC[-which(results_table_Offspring_DC$Outcome=="Developmental delay"),]
write.csv(results_table_Offspring_DC,"hypertension_subclass_offspring.csv" )
