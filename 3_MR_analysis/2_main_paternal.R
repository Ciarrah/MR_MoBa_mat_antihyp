...
outcomes_of_interest=gsub("_MBRN","",gsub("Beta_Pat_","",outcomes_of_interest))
...
results_all_DC[which(results_all_DC$Outcome%in%c("Hypertensive disorders of pregnancy", "Developmental delay")),c("Beta coefficient", "Upper CI", "Lower CI")]=
  exp(results_all_DC[which(results_all_DC$Outcome%in%c("Hypertensive disorders of pregnancy", "Developmental delay")),c("Beta coefficient", "Upper CI", "Lower CI")])
...
write.csv(results_all_DC ,"full_paternal_results.csv")

results_all_DC=read.csv("full_paternal_results.csv")
...
filename=paste0("hypertension_subclass_paternal_r&r.tiff")
...
results_table_Paternal_DC=results_table_Paternal_DC[-which(results_table_Paternal_DC$Outcome=="Developmental delay"),]
write.csv(results_table_Paternal_DC,"hypertension_subclass_paternal.csv" )
